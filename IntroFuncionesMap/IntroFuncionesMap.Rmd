...
* Usa `map()` para mapear `as_tibble()` sobre los vectores en `ldeaths`. Luego hacé clic en _Submit Answer_. (por ahora, usa `map()` en lugar de `map_dfr()`, esto te ayudará a ver lo que sucede paso a paso mientras trabajas con los datos).
...
3. Haga clic en _Submit Answer_.
...
* Usa `summarise_all()` y el código siguiente para sumar meses. Mira las sugerencias (hints) si te quedas trabado/a. Luego hacé clic en _Submit Answer_.
...
```{r all-check}
"¡Gran trabajo! Puedes ver un fuerte efecto estacional en los datos: en invierno ocurren más muertes relacionadas con los pulmones. Pudiste hacer esto debido al trabajo que dedicaste a trabajar los datos con purrr. ¡Date una palmadita en la espalda!"
```
